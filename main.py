#!/usr/bin/env python3
"""
WiFi Password List Generator for Bangladeshi Networks
...
# ------------------- Common Bangladeshi Data (embedded) -------------------
FIRST_NAMES = [
    "Rahim", "Karim", "Jamal", "Kamal", "Shahin", "Sharmin", "Nazmul", "Farhana",
    "Hasan", "Hossain", "Miah", "Khan", "Rahman", "Islam", "Akter", "Begum",
    "Mohammad", "Mohammed", "Ayesha", "Fatema", "Tania", "Runa", "Shahid", "Sohel",
    "Rubel", "Ripon", "Mamun", "Masud", "Parvez", "Shakil", "Shila", "Nargis",
    "Zahid", "Zahira", "Badal", "Bithi", "Liton", "Shanta", "Shuvro", "Tuhin"
]
...
DEFAULT_PASS = [
    "123456", "password", "12345678", "1234", "qwerty", "abc123", "111111",
    "123123", "admin", "letmein", "monkey", "iloveyou", "123456789", "000000",
    "11111111", "1234567890", "password1", "bangladesh", "dhaka123", "bismillah"
]

MOBILE_PREFIXES = ["017", "018", "019", "016", "015", "013", "014"]
YEARS = [str(y) for y in range(1980, 2026)]

# Leet speak substitutions (optional)
...
def add_common_variations(base_set, word, max_items):
    """Add word with suffixes, prefixes, and leet versions to base_set until limit."""
...
def generate_ssid_variations(ssid, username, max_items):
    """Generate password candidates based on SSID and username."""
    passwords = set()
...
# ------------------- Main generator -------------------
def main():
    parser = argparse.ArgumentParser(description="Generate a targeted WiFi password list.")
    ...
    # 3. Add common names and places
    for name in FIRST_NAMES:
        add_common_variations(passwords, name, max_pass)
        if len(passwords) >= max_pass:
            break
    ...
    # 5. Add mobile number patterns (simulated)
    if len(passwords) < max_pass:
    ...
    # 6. Add years with common prefixes
    if len(passwords) < max_pass:
        for year in YEARS:
            passwords.add(year)
            passwords.add("bangla" + year)
            passwords.add("dhaka" + year)
            if len(passwords) >= max_pass:
                break
